...
const Reports: React.FC<ReportsProps> = ({ 
  ...
  // --- CÁLCULO DE JUROS SEGURO (EVITA TRAVAMENTO) ---
  const calcularJurosAtraso = (dueDate: string, amount: any) => {
    ...
    const juros = valorBase * 0.015 * diasAtraso;
  ...
  const handleAction = async (loan: Loan, type: 'TOTAL' | 'ESTORNO', idx?: number) => {
    ...
    if (type === 'TOTAL' && idx !== undefined) {
      ...
      if (actualIdx === -1) return showToast("Sem parcelas pagas", "info");
      ...
      installments[actualIdx].amount = installments[actualIdx].value || (valor / 1.1); // Fallback caso perca valor original
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-6 shadow-2xl">
        ...
        {isAddingMovement && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6 p-4 bg-black/40 rounded-2xl border border-[#BF953F]/20">
            <select value={movementForm.type} onChange={e => setMovementForm({...movementForm, type: e.target.value as any})} className="bg-black border border-white/10 rounded-lg px-3 py-2 text-[10px] text-white outline-none">
              <option value="APORTE">APORTE</option>
              <option value="RETIRADA">RETIRADA</option>
            </select>
            <input placeholder="VALOR" value={movementForm.amount} onChange={e => setMovementForm({...movementForm, amount: e.target.value})} className="bg-black border border-white/10 rounded-lg px-3 py-2 text-[10px] text-white outline-none"/>
            <input placeholder="DESCRIÇÃO" value={movementForm.description} onChange={e => setMovementForm({...movementForm, description: e.target.value})} className="bg-black border border-white/10 rounded-lg px-3 py-2 text-[10px] text-white outline-none"/>
            <button onClick={() => {
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 bg-black border border-white/10 rounded-full px-4 py-2 flex items-center gap-2">
            <Search size={14} className="text-zinc-600"/><input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="PESQUISAR CLIENTE..." className="bg-transparent border-none text-[10px] text-white w-full outline-none uppercase"/>
          </div>
          <div className="flex bg-black p-1 rounded-xl border border-white/5">
            <button onClick={() => setFilterStatus('ATIVOS')} className={`px-6 py-1.5 rounded-lg text-[9px] font-black ${filterStatus === 'ATIVOS' ? 'bg-[#BF953F] text-black' : 'text-zinc-600'}`}>ATIVOS</button>
            <button onClick={() => setFilterStatus('FINALIZADOS')} className={`px-6 py-1.5 rounded-lg text-[9px] font-black ${filterStatus === 'FINALIZADOS' ? 'bg-[#BF953F] text-black' : 'text-zinc-600'}`}>PAGOS</button>
          </div>
        </div>
            ...
            <div key={loan.id} className="border border-white/5 rounded-[1.5rem] bg-black/20 overflow-hidden">
              <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div><h4 className="text-xs font-black text-white uppercase">{loan.customerName}</h4><p className="text-[9px] text-zinc-500">SALDO: R$ {((loan.totalToReturn || 0) - (loan.paidAmount || 0)).toFixed(2)}</p></div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleAction(loan, 'ESTORNO')} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all"><RotateCcw size={14}/></button>
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-6">
        <h3 className="text-xs font-black uppercase text-white mb-4 tracking-widest">Movimentações</h3>
        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          {(cashMovements || []).slice().reverse().map((t, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 rounded-xl">
              <div className="flex items-center gap-3">
                <div className={t.type === 'PAGAMENTO' || t.type === 'APORTE' ? 'text-emerald-500' : 'text-red-500'}><ArrowDownLeft size={16}/></div>
                <div><p className="text-[9px] font-black text-white uppercase">{t.description}</p><p className="text-[7px] text-zinc-600 uppercase">{new Date(t.date).toLocaleString()}</p></div>
              </div>
              <p className={`text-[10px] font-black ${t.type === 'PAGAMENTO' || t.type === 'APORTE' ? 'text-emerald-500' : 'text-red-500'}`}>R$ {Number(t.amount).toFixed(2)}</p>
            </div>
          ))}
        </div>
      </div>
