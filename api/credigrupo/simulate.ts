...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  try {
    ...
    const [customerSnapshot, borrowerSnapshot] = await Promise.all([
      ...
      adminDb.doc(`creditBorrowers/${borrowerLinkId(customerId, CREDIGRUPO_ACCOUNT_MODE, environment)}`).get(),
    ]);
    ...
    requireCredigrupoRecordForEnvironment(borrower, environment);
